...
int main(int argc, char ** argv) {
  ...
  if (argc!=5) {
	...
	if (local[0]<=pad_size[0] || local[1]<=pad_size[1])
		{if (rank==0){printf("Cannot divide equally the resources--BYE \n");} 
		MPI_Finalize();
		exit(-1);}
	...
	if (west==0 && east==0 && south==0)
	{
		...
		{
		i_min=1;
		i_max=local[0]-1-pad_size[0]; 		j_min=2;	
		j_max=local[1]-1;
		if (flag[1]==1) j_max=local[1]-1-pad_size[1];
		}
	}
...
MPI_Request recv_request[4],send_request[4];
MPI_Status status[4];
int nmsgs;
...
for (t=0;t<T && !global_converged;t++) {
	...
	if (east==1)
	{	
	MPI_Isend(&(u_previous[0][local[1]]),1,column,rank+1,rank,MPI_COMM_WORLD,&send_request[nmsgs]);
	MPI_Irecv(&(u_previous[0][local[1]+1]),1,column,rank+1,rank+1,MPI_COMM_WORLD,&recv_request[nmsgs]) ;
	nmsgs++;
	}
	...
	if (west==1)
	{
	MPI_Isend(&(u_previous[0][1]),1,column,rank-1,rank,MPI_COMM_WORLD,&send_request[nmsgs]);
	MPI_Irecv(&(u_previous[0][0]),1,column,rank-1,rank-1,MPI_COMM_WORLD,&recv_request[nmsgs]) ;
	nmsgs++;
	}
 
	if (north==1)
	{
	MPI_Isend(&(u_previous[1][0]),local[1]+2,MPI_DOUBLE,rank-grid[1],rank,MPI_COMM_WORLD,&send_request[nmsgs]);
	MPI_Irecv(&(u_previous[0][0]),local[1]+2,MPI_DOUBLE,rank-grid[1],rank-grid[1],MPI_COMM_WORLD,&recv_request[nmsgs]) ;
	nmsgs++;
	}

	if (south==1)
	{
	MPI_Isend(&(u_previous[local[0]][0]),local[1]+2,MPI_DOUBLE,rank+grid[1],rank,MPI_COMM_WORLD,&send_request[nmsgs]);
	MPI_Irecv(&(u_previous[local[0]+1][0]),local[1]+2,MPI_DOUBLE,rank+grid[1],rank+grid[1],MPI_COMM_WORLD,&recv_request[nmsgs]) ;
	nmsgs++;
	}
	MPI_Waitall(nmsgs, &send_request[0], &status[0]);
	MPI_Waitall(nmsgs, &recv_request[0], &status[0]);
