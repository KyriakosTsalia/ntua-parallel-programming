...
int main(int argc, char **argv)
{
     ...
     */ 
		...
		dim3 gpu_grid(grid_size,1);    // FILLME: set up the grid dimensions
		size_t shmem_size = 0;
		if (kernel==2) 	{
			shmem_size = n*sizeof(value_t);  // FILLME: set up the shared memory size
			if (shmem_size>48*1024) shmem_size= 48*1024;
		}
    ...
    for (size_t i = 0; i < NR_ITER; ++i) {
		...
		cublasDgemv('n',n,n,1.0,(double *)gpu_A,n,(double *)gpu_x,1,0.0,(double *)gpu_y,1);
